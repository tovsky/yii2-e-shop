...
//        debug($this->tree);                              // Распечатываем для отладки, просматриваем разные моменты.
    ...
    protected function getMenuHtml($tree, $tab = '')       // Метод проходится в цикле по дереву. Берет каждый элемент и передает затем параметром
    {
        $str = '';                              // в эту переменную будем помещать готовый HTML код
        foreach ($tree as $category) {
            $str .= $this->catToTemplate($category, $tab);    //  в метод catToTemplate   каждый элемент дерева передается параметром
        }
        return $str;
    }

    protected function catToTemplate($category, $tab)     // Метод принимает параметром переданный элемент, помещает его в шаблон
    {
